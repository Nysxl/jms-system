...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  return res.status(501).json({ error: 'Email functionality requires resend package. Install with: npm install resend' });
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const { reportId, recipientEmail, message } = req.body;
